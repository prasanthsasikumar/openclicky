...
to HeyClicky itself) and Codex talks to it through a thin stdio proxy.
...
The overlay lives in HeyClicky's embedded daemon; no setup needed.
...
| `Screen Recording permission not granted` | TCC not granted for capture | Affects `get_window_state` / `get_desktop_state` capture. Grant to Clicky.app in System Settings — the driver can't operate without it |
