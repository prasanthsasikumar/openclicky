...
## Primary Path
1. Decide the delivery format first. If the result can be one self-contained HTML file (with inline or CDN CSS/JS), build it as a static file and hand back the absolute path via `openclicky-artifacts`. Do not start a server for something a `file://` open can show.
...
1. Start it detached and logged:
   `nohup npm run preview -- --host 127.0.0.1 --port 4173 > .openclicky-preview.log 2>&1 &`
...
3. If the poll never succeeds, `tail -n 40 .openclicky-preview.log` and report the real error — do not hand back a URL you have not seen respond.
...
## Fallbacks
- For one-off pages, create a single HTML/CSS/JS file and open/reveal it via `openclicky-artifacts`. This is the default, not the fallback.
