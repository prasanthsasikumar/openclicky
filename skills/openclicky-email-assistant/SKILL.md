...
## Do Not Use When
- The task is only reading Google Drive/Docs/Sheets or Calendar; use `openclicky-google-workspace`.
...
2. Use `openclicky-google-workspace` through Composio first for Gmail and Google contacts when the integration is attached.
...
- If a Gmail draft/send tool shape is unknown or ambiguous, call `COMPOSIO_GET_TOOL_SCHEMAS` once for the exact Gmail tool slugs and use the returned key names. OpenClicky's Worker caches schema responses for about 10 minutes.
...
- If a contact list is in CSV/XLSX/Sheets, use `spreadsheet` or `openclicky-google-workspace` to inspect it before drafting.
