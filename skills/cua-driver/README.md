...
change their browser profile. OpenClicky supervises an embedded
`cua-driver` daemon inside `OpenClicky.app` (the MCP entry is a thin stdio
proxy to it), inherits OpenClicky's Accessibility and Screen Recording
...
3. **Bundled driver present**:
   `OpenClicky.app/Contents/Helpers/cua-driver`.
...
docs, then re-apply OpenClicky's managed-runtime guardrails.
