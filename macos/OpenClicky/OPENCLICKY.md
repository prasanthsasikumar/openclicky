...
| `FrontmostAppObserver.swift` (new) | At push-to-talk key-down: `NSWorkspace` frontmost app; for browsers the front tab's URL through Accessibility (Safari `AXDocument`, else a bounded search for `AXWebArea` → `AXURL`, 0.25 s messaging timeout) and the window title. Needs the Accessibility permission for URLs; falls back to the browser's own app skill without it. |
