...
| `AppSkillMatcher.swift` (new) | `FrontAppContext` (bundle id, app name, URL, window title) and the pure matcher: site by URL host (exact or subdomain), then site by window title, then app by bundle id; the most specific site wins. |
...
| `FrontmostAppObserver.swift` (new) | At push-to-talk key-down: `NSWorkspace` frontmost app; for browsers the front tab's URL through Accessibility (Safari `AXDocument`, else a bounded search for `AXWebArea` → `AXURL`, 0.25 s messaging timeout) and the window title (title matching applies to browsers only). Both need the Accessibility permission; without it only the bundle id and app name are known and the browser's own app skill is used. |
