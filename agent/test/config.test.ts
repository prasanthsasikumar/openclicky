...
describe("resolveConfig", () => {
  it("applies defaults", () => {
    const c = resolveConfig({}, { HOME: "/home/u" });
  ...
  it("env then flags win", () => {
    const c = resolveConfig(
      { token: "flag" },
      { HOME: "/h", BACKEND_URL: "http://b:1/", OPENCLICKY_TOKEN: "env", OPENCLICKY_WORKSPACE: "/ws", OPENCLICKY_MODEL: "m" },
    );
  ...
  describe("runTimeoutMs", () => {
    it("defaults to 10 minutes", () => {
      expect(resolveConfig({}, { HOME: "/home/u" }).runTimeoutMs).toBe(10 * 60 * 1000);
    });
    it("takes an override from the env", () => {
      expect(resolveConfig({}, { HOME: "/home/u", OPENCLICKY_RUN_TIMEOUT_MS: "5000" }).runTimeoutMs).toBe(5000);
    });
    it("ignores a non-positive env value and falls back to the default", () => {
      expect(resolveConfig({}, { HOME: "/home/u", OPENCLICKY_RUN_TIMEOUT_MS: "not-a-number" }).runTimeoutMs).toBe(10 * 60 * 1000);
      expect(resolveConfig({}, { HOME: "/home/u", OPENCLICKY_RUN_TIMEOUT_MS: "0" }).runTimeoutMs).toBe(10 * 60 * 1000);
    });
    it("lets a flag win over the env", () => {
      expect(resolveConfig({ runTimeoutMs: 42 }, { HOME: "/home/u", OPENCLICKY_RUN_TIMEOUT_MS: "5000" }).runTimeoutMs).toBe(42);
    });
  });
});
